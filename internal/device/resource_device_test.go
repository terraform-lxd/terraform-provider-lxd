...
func TestAccDevice_volumeAttach(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccDevice_volumeAttach(poolName, volumeName, instanceName),
				Check: resource.ComposeTestCheckFunc(
...
func testAccDevice_volumeAttach(poolName string, volumeName string, instanceName string) string {
	return fmt.Sprintf(`
