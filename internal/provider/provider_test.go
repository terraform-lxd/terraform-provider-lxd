		...
		Steps: []resource.TestStep{
			{
				...
				ExpectError: regexp.MustCompile(`Server certificate fingerprint mismatch`),
			},
		},
	})
}
		...
		Steps: []resource.TestStep{
			{
				...
				ExpectError: regexp.MustCompile(`Attribute "bearer_token" cannot be specified when`),
				PlanOnly:    true,
			},
		},
	})
		...
		Steps: []resource.TestStep{
			{
				...
				ExpectError: regexp.MustCompile(`Both "client_certificate" and "client_key" must be provided`),
			},
			{
