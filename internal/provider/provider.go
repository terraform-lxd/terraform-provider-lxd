...
import (
	...
	"github.com/maveonair/terraform-provider-incus/internal/image"
	"github.com/maveonair/terraform-provider-incus/internal/instance"
	"github.com/maveonair/terraform-provider-incus/internal/network"
	"github.com/maveonair/terraform-provider-incus/internal/profile"
	"github.com/maveonair/terraform-provider-incus/internal/project"
	provider_config "github.com/maveonair/terraform-provider-incus/internal/provider-config"
	"github.com/maveonair/terraform-provider-incus/internal/storage"
)
				...
				NestedObject: schema.NestedBlockObject{
					Attributes: map[string]schema.Attribute{
						...
						"address": schema.StringAttribute{
							Optional:    true,
							Description: "The FQDN or IP where the Incus daemon can be contacted. (default = \"\" (read from incus config))",
						},
...
func (p *IncusProvider) Configure(ctx context.Context, req provider.ConfigureRequest, resp *provider.ConfigureResponse) {
	...
	for _, remote := range data.Remotes {
		...
		incusRemote := provider_config.IncusProviderRemoteConfig{
		...
		incusProvider.SetRemote(incusRemote, isDefault)
	}
