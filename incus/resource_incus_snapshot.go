...
func resourceIncusSnapshot() *schema.Resource {
	return &schema.Resource{
		Create: resourceIncusSnapshotCreate,
		Delete: resourceIncusSnapshotDelete,
		Exists: resourceIncusSnapshotExists,
		Read:   resourceIncusSnapshotRead,
...
func resourceIncusSnapshotCreate(d *schema.ResourceData, meta interface{}) error {
	...
	return resourceIncusSnapshotRead(d, meta)
}

func resourceIncusSnapshotRead(d *schema.ResourceData, meta interface{}) error {
	p := meta.(*incusProvider)
...
func resourceIncusSnapshotDelete(d *schema.ResourceData, meta interface{}) error {
	p := meta.(*incusProvider)
	server, err := p.GetInstanceServer(p.selectRemote(d))
	if err != nil {
		return err
...
func resourceIncusSnapshotExists(d *schema.ResourceData, meta interface{}) (bool, error) {
	p := meta.(*incusProvider)
	server, err := p.GetInstanceServer(p.selectRemote(d))
	if err != nil {
		return false, err
...
func (s snapshotID) IncusID() string {
