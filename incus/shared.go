...
// Helper functions.
func resourceIncusConfigMap(c interface{}) map[string]string {
	...
	log.Printf("[DEBUG] Incus Configuration Map: %#v", config)

	return config
}

// resourceIncusConfigMapAppend appends a map of configuration values
// to an existing map. All appended config values are prefixed
// with the config namespace.
func resourceIncusConfigMapAppend(config map[string]string, append interface{}, namespace string) map[string]string {
	if config == nil {
		panic("config is nil")
	}
...
func resourceIncusDevices(d interface{}) map[string]map[string]string {
	...
	log.Printf("[DEBUG] Incus Devices: %#v", devices)

	return devices
}

func resourceIncusNetworkZoneRecordEntries(r interface{}) []api.NetworkZoneRecordEntry {
...
func resourceIncusValidateDeviceType(v interface{}, k string) (ws []string, errors []error) {
	validTypes := []string{
		"none", "disk", "nic", "unix-char", "unix-block", "usb", "gpu", "infiniband", "proxy", "unix-hotplug", "tpm", "pci",
	}
...
func resourceIncusValidateInstanceType(v interface{}, k string) (ws []string, errors []error) {
...
func resourceIncusValidateNetworkType(v interface{}, k string) (ws []string, errors []error) {
...
func instanceUploadFile(server incus.InstanceServer, instance string, file File) error {
...
func instanceDeleteFile(server incus.InstanceServer, instance string, targetFile string) error {
...
// recursiveMkdir was copied almost as-is from github.com/lxc/incus/blob/main/cmd/incus/file.go.
func recursiveMkdir(d incus.InstanceServer, instance string, p string, mode os.FileMode, uid int64, gid int64) error {
