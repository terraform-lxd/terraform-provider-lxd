...
resource "openstack_compute_keypair_v2" "incus" {
  name       = "incus"
  public_key = tls_private_key.local.public_key_openssh
}

resource "openstack_compute_instance_v2" "incus" {
  name            = "incus"
  image_name      = "Ubuntu 16.04"
  flavor_name     = "m1.medium"
  key_pair        = openstack_compute_keypair_v2.incus.name
  security_groups = ["AllowAll"]
  user_data       = "#cloud-config\ndisable_root: false"
}

resource "null_resource" "incus" {
  connection {
    user        = "root"
    private_key = tls_private_key.local.public_key_openssh
    host        = openstack_compute_instance_v2.incus.access_ip_v6
  }
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "echo '${tls_self_signed_cert.ca.cert_pem}' | sudo tee /var/lib/incus/server.ca",
      "echo '${tls_locally_signed_cert.incus.cert_pem}' | sudo tee /var/lib/incus/server.crt",
      "echo '${tls_self_signed_cert.ca.cert_pem}' | sudo tee -a /var/lib/incus/server.crt",
      "echo '${tls_private_key.incus.private_key_pem}' | sudo tee /var/lib/incus/server.key",
...
resource "tls_private_key" "incus" {
  algorithm = "RSA"
}

resource "tls_cert_request" "incus" {
  key_algorithm   = "RSA"
  private_key_pem = tls_private_key.incus.private_key_pem
  ip_addresses    = ["${replace(openstack_compute_instance_v2.incus.access_ip_v6, "/[][]/", "")}"]

  subject {
    common_name = openstack_compute_instance_v2.incus.name
  }
}

resource "tls_locally_signed_cert" "incus" {
  cert_request_pem = tls_cert_request.incus.cert_request_pem
...
output "hostname" {
  value = openstack_compute_instance_v2.incus.access_ip_v6
}
