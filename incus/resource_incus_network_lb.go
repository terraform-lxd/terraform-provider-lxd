...
func resourceIncusNetworkLB() *schema.Resource {
	return &schema.Resource{
		Read:   resourceIncusNetworkLBRead,
		Create: resourceIncusNetworkLBCreate,
		Update: resourceIncusNetworkLBUpdate,
		Delete: resourceIncusNetworkLBDelete,
		Exists: resourceIncusNetworkLBExists,
...
func resourceIncusNetworkLBRead(d *schema.ResourceData, meta interface{}) error {
...
func resourceIncusNetworkLBCreate(d *schema.ResourceData, meta interface{}) error {
	p := meta.(*incusProvider)
	...
	req.Config = resourceIncusConfigMap(d.Get("config"))
	req.Backends = resourceIncusNetworkLBBackends(d.Get("backend"))
	req.Ports = resourceIncusNetworkLBPorts(d.Get("port"))
	...
	return resourceIncusNetworkLBRead(d, meta)
}

func resourceIncusNetworkLBUpdate(d *schema.ResourceData, meta interface{}) error {
	p := meta.(*incusProvider)
	server, err := p.GetInstanceServer(p.selectRemote(d))
	if err != nil {
		return err
...
func resourceIncusNetworkLBDelete(d *schema.ResourceData, meta interface{}) error {
	p := meta.(*incusProvider)
	server, err := p.GetInstanceServer(p.selectRemote(d))
	if err != nil {
		return err
...
func resourceIncusNetworkLBExists(d *schema.ResourceData, meta interface{}) (exists bool, err error) {
	p := meta.(*incusProvider)
	server, err := p.GetInstanceServer(p.selectRemote(d))
	if err != nil {
		return false, err
...
func resourceIncusNetworkLBBackends(d interface{}) []api.NetworkLoadBalancerBackend {
	...
	log.Printf("[DEBUG] Incus Backends: %#v", backends)

	return backends
}

func resourceIncusNetworkLBPorts(d interface{}) []api.NetworkLoadBalancerPort {
	...
	log.Printf("[DEBUG] Incus Ports: %#v", ports)
