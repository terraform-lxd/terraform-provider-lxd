...
- some Incus stateful snapshot bugs are isolated and resolved / worked around
e.g.
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr("incus_snapshot.snapshot1", "name", snap1Name),
				),
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr("incus_snapshot.snapshot1", "name", snap1Name),
					resource.TestCheckResourceAttr("incus_snapshot.snapshot2", "name", snap2Name),
				),
			},
		},
	})
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					testAccSnapshotExistsInProject(t, "incus_snapshot.snapshot1", &snap, projectName),
				),
			},
		},
	})
...
func TestSnapshotId_IncusID(t *testing.T) {
	type fields struct {
		remote    string
		container string
		snapshot  string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if got := s.IncusID(); got != tt.want {
				t.Errorf("snapshotId.IncusID() = %v, want %v", got, tt.want)
			}
		})
	}
}
...
resource "incus_snapshot" "snapshot1" {
  container_name = "${incus_instance.instance1.name}"
  name = "%s"
  stateful = "%v"
}
