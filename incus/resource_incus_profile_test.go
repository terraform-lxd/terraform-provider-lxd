		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr("incus_profile.profile1", "name", profileName),
					resource.TestCheckTypeSetElemNestedAttrs("incus_profile.profile1", "device.*", map[string]string{"properties.path": "/tmp/shared"}), testAccProfileRunning(t, "incus_profile.profile1", &profile),
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr("incus_profile.profile1", "name", profileName),
					resource.TestCheckTypeSetElemNestedAttrs("incus_profile.profile1", "device.*", map[string]string{"properties.path": "/tmp/shared"}),
					testAccProfileRunning(t, "incus_profile.profile1", &profile),
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					testAccProfileRunningInProject(t, "incus_profile.profile1", &profile, projectName),
				),
			},
		},
	})
