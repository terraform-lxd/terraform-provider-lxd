...
func resourceIncusInstance() *schema.Resource {
	return &schema.Resource{
		Create: resourceIncusInstanceCreate,
		Update: resourceIncusInstanceUpdate,
		Delete: resourceIncusInstanceDelete,
		Exists: resourceIncusInstanceExists,
		Read:   resourceIncusInstanceRead,
		Importer: &schema.ResourceImporter{
			State: resourceIncusInstanceImport,
		},
		...
		Schema: map[string]*schema.Schema{
			...
			"type": {
				...
				Optional:     true,
				Computed:     true,
				ValidateFunc: resourceIncusValidateInstanceType,
			},
...
func resourceIncusInstanceCreate(d *schema.ResourceData, meta interface{}) error {
	var err error
	...
	refreshInterval := meta.(*incusProvider).RefreshInterval
	...
	config = resourceIncusConfigMapAppend(config, d.Get("limits"), "limits.")
	...
	if d.Get("start_on_create").(bool) {
		...
		if err != nil {
			...
			return fmt.Errorf("Incus server rejected request to start instance (%s): %s", name, err)
		}
		...
		stateConf := &retry.StateChangeConf{
			...
			Refresh:    resourceIncusInstanceRefresh(server, name),
		...
		if d.Get("wait_for_network").(bool) {
			// Incus will return "Running" even if "inet" has not yet been set.
			// wait until we see an "inet" ip_address before reading the state.
			networkConf := &retry.StateChangeConf{
				Target:     []string{"OK"},
				Refresh:    resourceIncusInstanceWaitForNetwork(server, name),
				Timeout:    3 * time.Minute,
				Delay:      refreshInterval,
				MinTimeout: 3 * time.Second,
			}
	...
	return resourceIncusInstanceRead(d, meta)
}

func resourceIncusInstanceRead(d *schema.ResourceData, meta interface{}) error {
	p := meta.(*incusProvider)
	...
	name := d.Id()
	// https://github.com/lxc/incus/blob/main/client/incus_instances.go
	...
	if instance.Type == "" {
		// If the Incus server does not support virtualization (e.g. because not
	...
	for name, incusdevice := range instance.Devices {
...
func resourceIncusInstanceUpdate(d *schema.ResourceData, meta interface{}) error {
	p := meta.(*incusProvider)
	...
	// https://github.com/lxc/incus/blob/3df4aa84e8a86f5186b312243dc212ff8da06941/shared/api/instance.go#L136
	...
	if d.HasChanges("config") {
		...
		oldConfigMap := resourceIncusConfigMap(oldConfig)
		newConfigMap := resourceIncusConfigMap(newConfig)
...
func resourceIncusInstanceDelete(d *schema.ResourceData, meta interface{}) (err error) {
	p := meta.(*incusProvider)
	...
	// Wait for the Incus instance to be deleted
	err = op.Wait()
	if err != nil {
		return err
	}
...
func resourceIncusInstanceExists(d *schema.ResourceData, meta interface{}) (exists bool, err error) {
	p := meta.(*incusProvider)
...
func resourceIncusInstanceImport(d *schema.ResourceData, meta interface{}) ([]*schema.ResourceData, error) {
	p := meta.(*incusProvider)

	log.Printf("[DEBUG] Starting import for %s", d.Id())
	parts := strings.SplitN(d.Id(), "/", 2)

	remote, name, err := p.IncusConfig.ParseRemote(parts[0])
	if err != nil {
		return nil, err
	}
...
func resourceIncusInstanceRefresh(server incus.InstanceServer, name string) retry.StateRefreshFunc {
...
func resourceIncusInstanceWaitForNetwork(server incus.InstanceServer, name string) retry.StateRefreshFunc {
	return func() (interface{}, string, error) {
		st, _, err := server.GetInstanceState(name)
		if err != nil {
			return st, "Error", err
